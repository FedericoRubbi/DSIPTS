...
class TTM(Base):
    ...
    def __init__(self, 
        ...
        if self.use_quantiles:
            ...
            if self.out_channels * 3 != len(prediction_channel_indices):
                #, exogenous_channel_indices, num_input_channels = self.__add_quantile_features(prediction_channel_indices, 
                #                                                                                                         exogenous_channel_indices, 
                #                                                                                                         self.out_channels)
                
                self.past_channels += 2*self.out_channels
                prediction_channel_indices = list(range(3*self.out_channels))
        ...
        self.model = get_model(
            ...
            num_input_channels=self.past_channels+len(self.embs_past),
            decoder_mode=decoder_mode,
            prediction_channel_indices=prediction_channel_indices,
            exogenous_channel_indices=None,#exogenous_channel_indices,
    ...
    def __scaler_past(self, input):
        for i, e in enumerate(self.embs_past):
            input[:,:,i] = input[:, :, i] / (e-1)
        return input
    def __scaler_fut(self, input):
    ...
    def __permute_indexes(self, values, target_idx, current_idx):
        ...
        if sorted(current_idx) != sorted(target_idx):
            t = self.__build_tupla_indexes(values.shape[-1], target_idx, current_idx)
    ...
    def forward(self, batch):
        ...
        #original_indexes_future = original_indexes
        ...
        if 'x_cat_past' in batch.keys():
            ...
            x_mark_enc = self.__scaler_past(x_mark_enc)
        ...
        if 'x_cat_future' in batch.keys():
            ...
            x_mark_dec = self.__scaler_fut(x_mark_dec)
        ...
        res = self.model(
            ...
            future_values= future_values if future_values.shape[0]>0 else None,
