from dsipts import RNN, LinearTS, Persistent, D3VAE, DilatedConv, TFT, Informer,VVA,VQVAEA,CrossFormer,Autoformer,PatchTST,Diffusion,DilatedConvED,TIDE,ITransformer,TimeXER,beauty_string, TTM,Samformer,Duet, Simple
import numpy as np
...
def select_model(conf, model_conf,ts):
    ...
    if conf.model.type == 'linear':
        ...
        model =  Duet(**model_conf,   optim_config = conf.optim_config,
                          scheduler_config =conf.scheduler_config,verbose=ts.verbose )
    elif conf.model.type == 'simple':
        model =  Simple(**model_conf,   optim_config = conf.optim_config,
                          scheduler_config =conf.scheduler_config,verbose=ts.verbose )
    else:
        model = None
...
def load_model(ts,conf):
    ...
    if conf.model.type == 'linear':
        ...
        ts.load(Simple,os.path.join(conf.train_config.dirpath,'model'),load_last=conf.inference.load_last)
